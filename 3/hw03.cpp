/*
  ...
  The goal of this program is to write a program that prints using iterator when it calls a functor that is true.
*/
...
SCENARIO("Testing Print_If"){
  ...
  GIVEN("==========My Own Unit Test==========\n A Functor that takes floats"){
    WHEN("Functor foo is used for print_if") {
      list<float> fl;
      for(float x = 8.2; x < 50; ++x){
	fl.push_front(x);
      }

      THEN("When the vaue is greater than 31.2"){
	float findF = 31.2;
	cout_redirect redirect(buffer.rdbuf());
	foo<float> fbar(findF);
	print_if(fl.begin(), fl.end(), fbar);
	vector<float> resultFVector = toVec<float>(buffer);
	REQUIRE(resultFVector.size() != 0);
	for(float& each:resultFVector){
	  REQUIRE(each > 31.2); 
	}
      }
    }
