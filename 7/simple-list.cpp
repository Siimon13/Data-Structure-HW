...
public:
  ...
  List( const List & rhs ){
    header = new Node();
    typename List<Object>::iterator rhsitr = rhs.begin();
    push_front(*rhsitr);
    typename List<Object>::iterator currentitr = begin();

    while(++rhsitr != nullptr)
      currentitr = insert_after(currentitr, *rhsitr);
  ...
  void merge( List & alist ){
    typename List<Object>::iterator alistitr = alist.begin();
    
    typename List<Object>::iterator currentitr = begin();
    typename List<Object>::iterator tempitr;

    while(currentitr.current->next != nullptr && alistitr.current != nullptr){

      tempitr = currentitr;
      if (*currentitr++ < *alistitr && *currentitr > *alistitr){
	insert_after(tempitr, *alistitr++);
	currentitr = tempitr;
      }
    }

    // while (alistitr.current != nullptr){

    //   tempitr = insert_after(tempitr, *alistitr++);
  ...
  void remove_adjacent_duplicates( ){
    ...
    while(currentitr.current->next != nullptr){
      tempitr = currentitr;
      if(*currentitr++ == *currentitr){
	erase_after(tempitr);
	currentitr = tempitr;
      }
    }
  ...
  void remove_if( Predicate pred ){
    typename List<Object>::iterator currentitr = before_begin();
    typename List<Object>::iterator tempitr;
